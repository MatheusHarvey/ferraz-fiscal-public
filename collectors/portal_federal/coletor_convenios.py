...
def main():
    logger.info("Iniciando coleta de convênios de Ferraz de Vasconcelos...")
    todos = []
    pagina = 1
    ...
    if todos:
        ...
        saida = RAW_DIR / "convenios_ferraz.csv"
        df.to_csv(saida, index=False, encoding="utf-8-sig", sep=";")
        logger.success(f"Total: {len(df)} convênios salvos em {saida}")

        # Resumo rápido
        print(f"\nTotal de convênios: {len(df)}")
